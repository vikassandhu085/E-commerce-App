...
}).post(function(req,res)
{
  ...
  if(username && password && email && req.file && rePassword===password)
  {
    ...
    }).then(function(user)
    {
      ...
      var url = `<h1>Hi ${user.username} Thanks for Joining Us </h1> <a href="https://e-commerce-app-3p34g8mlvel1t7noeq.codequotient.in/verifyUser/${user._id}">click here to verify your account</a>`
      
      sendMail(email , "Welcome to Alibaba", "click here to verify", url , function(error)
      {
        if(error)
        {
